...
public class CloudFrontTransferHelper implements TransferHelper, HttpDownloadListener {
    ...
    private boolean startTransfer(final String relativeFilePath, final long fileSize,
        ...
        final URI uri;
        try {
            uri = new URI("http://" + cloudFrontDomain + "/" + Uri.encode(cloudFrontUrlPrefix + relativeFilePath));
    ...
    public synchronized void download(final String relativeFilePath, final long fileSize,
        ...
        if (downloadId == null) {
            try {
                // download the item.
                if (!startTransfer(relativeFilePath, fileSize, listener)) {
                    return;
                }
     ...
     */
    private void cleanUpDownload(final HttpDownloadObserver observer) {
        final long downloadId = observer.getId();
    ...
    @Override
    public void onStateChanged(final long id, final DownloadState state) {
    ...
    @Override
    public void onProgressChanged(final long id, final long bytesCurrent, final long bytesTotal) {
        final ContentProgressListener listener = progressListeners.get(id);
    ...
    @Override
    public void onError(final long id, final HttpDownloadException ex) {
        final HttpDownloadObserver observer = downloadsInProgress.get(id);
        Log.d(LOG_TAG, ex.getMessage(), ex);
        final ContentProgressListener listener = progressListeners.get(id);
        if (listener != null) {
